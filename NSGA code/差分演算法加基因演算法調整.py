...
iteration_makespan=[]
iteration_Scrap=[]
new_collect_chromosome_mqakespan=np.zeros((Num_chromosome*2,3))
...
for k in range(1,iteration+1):
    ...
    if k == 1:
        ...
        #變異因子===========================================================================
        for i_z in range(0,100,5):
            NP=5#群駔
            F=0.5#延伸或降低因子
            initialization_chromosome_NP=np.zeros((NP*2,Num_job*2))
            lenght=np.arange(len(initialization_chromosome_NP)/2)
            for i in range(0,5):
                for j in range(Num_job*2):
                    initialization_chromosome_NP[i][0:]=initialization_chromosome[i+i_z][0:]
                    for j in range(Num_job*2):
                        random.shuffle(lenght)
                        for i in range(5,10):
                             random.shuffle(lenght)
                             NEW_GENE=initialization_chromosome[int(lenght[0])][j]+F*(initialization_chromosome[int(lenght[1])][j]-initialization_chromosome[int(lenght[2])][j])
                             if NEW_GENE<=0:
                                 NEW_GENE=NEW_GENE*-1
                                 initialization_chromosome_NP[i][j]=NEW_GENE
                             elif NEW_GENE>1:
                                 NEW_GENE=np.random.rand()
                                 initialization_chromosome_NP[i][j]=NEW_GENE
                             else:
                                 initialization_chromosome_NP[i][j]=NEW_GENE
            ...
            for z in range(0,10):
                ...
                for i in range(Num_job):
                    for j in range(1,Num_Machine):
                        ...
                        if division_number*weight_number<initialization_chromosome_NP[z][i]:
                            weight_number+=1
                        elif division_number*weight_number>initialization_chromosome_NP[z][i]:
                            break;
                    ...
                    select_machine[i][3]=initialization_chromosome_NP[z][100+i]
                    weight_number=1
            ...
            collect_chromosome_mqakespan_saveplace=np.zeros((10,4))
            ...
            for i in range(0,5):
                new_collect_chromosome_mqakespan[i+i_z][0]=i+i_z
                new_collect_chromosome_mqakespan[i+i_z][1]=collect_chromosome_mqakespan.iloc[i][0]
                new_collect_chromosome_mqakespan[i+i_z][2]=collect_chromosome_mqakespan.iloc[i][2]
            collect_chromosome_mqakespan.sort_values(by=['完工時間'], ascending=(True) ,inplace=True)
            
            for i in range(0,5):
                initialization_chromosome[i+100+i_z][0:]=initialization_chromosome_NP[collect_chromosome_mqakespan.index[i]][0:]
                new_collect_chromosome_mqakespan[i+100+i_z][0]=i+100+i_z
                new_collect_chromosome_mqakespan[i+100+i_z][1]=collect_chromosome_mqakespan.iloc[i][0]
                new_collect_chromosome_mqakespan[i+100+i_z][2]=collect_chromosome_mqakespan.iloc[i][2]
        new_collect_chromosome_mqakespan=pd.DataFrame(new_collect_chromosome_mqakespan)
        new_collect_chromosome_mqakespan.sort_values(by=1, ascending=(True) ,inplace=True)
        ...
        iteration_makespan.append(new_collect_chromosome_mqakespan.iloc[0][1])
        
        NSGA_TEST = new_collect_chromosome_mqakespan.iloc[:,[1,2]]
        import pymoo
        import NSGAII
        ...
        for i in range(int(Num_chromosome*Elite_rate),100):
            initialization_chromosome_select[i][0:200]=initialization_chromosome[test_NSGA_TEST.index[i-int(Num_chromosome*Elite_rate)],0:200]
