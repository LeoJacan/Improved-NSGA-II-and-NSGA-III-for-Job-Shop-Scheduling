# -*- coding: utf-8 -*-
"""
Created on Sun Dec 12 17:33:16 2021
...
for k in range(1,iteration+1):
    ...
    if k == 1:
        ...
        for i in range(int((Num_chromosome*cross_rate)/2)):
        ...
        #突變============================================================================
            mutation_rows=0
            lenght2=np.arange(Num_chromosome+(Num_chromosome*cross_rate))
            random.shuffle(lenght2)
            i+=1
            parents_C = lenght2[0:int((Num_chromosome*mutation_rate)/2)]
            parents_D = lenght2[int((Num_chromosome*mutation_rate)/2):]
            for x in range(0,int((Num_chromosome*mutation_rate)/2)):
        ...
        import pymoo
        import NSGAIII
        test_NSGA_TEST=NSGAIII.ValuesCal(NSGA_TEST,200)
        #test_NSGA_TEST = NSGAIII.RankforNSGAIII(NSGA_TEST)
        print(test_NSGA_TEST.iloc[0])
        initialization_chromosome_select_nb=[]
        initialization_chromosome_select=np.zeros((Num_chromosome,Num_job*2))#新的母代
        test_NSGA_TEST_2=test_NSGA_TEST.sort_values("完工時間", ascending=True)
        for i in range(0,int(Num_chromosome*Elite_rate)):
                initialization_chromosome_select_nb.append(test_NSGA_TEST_2.index[i])
                initialization_chromosome_select[i][0:200]=initialization_chromosome[test_NSGA_TEST_2.index[i],0:200]
        ...
        for i in range(Num_chromosome*2-1,int(Num_chromosome*Elite_rate)-1,-1):
            if test_NSGA_TEST_2.iloc[i][5]!=test_NSGA_TEST_2.iloc[i-1][5]:
                nsga_ranknb[i-int(Num_chromosome*Elite_rate),0]=x
                x+=1
            elif test_NSGA_TEST_2.iloc[i][5]==test_NSGA_TEST_2.iloc[i-1][5]:
                nsga_ranknb[i-int(Num_chromosome*Elite_rate),0]=x
